...
const Breakfast = () => {
  ...
  const BreakfastProducts = productsToShow.filter(
    (item) => item.category === "Breakfast & Instant Food");
  // Group products by subcategory
  // groups the breakfastProducts array by their subcategory.
  // key is a subcategory name,
  // value is an array of products belonging to that subcategory.
  // For each product (item), we check if acc already has an array for its subcategory.
  // { "Deserts": [product1, product2], "Noudles": [product3], ... }
  const groupedBySubcategory = BreakfastProducts.reduce((acc, item) => {
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-2 sm:px-4">
      <div className="mb-8 text-center">
        <h3 className="text-3xl font-extrabold text-green-700 tracking-tight">Breakfast & Instant Food</h3>
      </div>
